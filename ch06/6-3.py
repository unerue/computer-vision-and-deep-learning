...
class Orim(QMainWindow):
    ...
    def painting(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            cv2.circle(
                self.img_show, (x, y), self.BrushSiz, self.LColor, -1
            )  # 왼쪽 버튼을 클릭하면 파란색
            cv2.circle(self.mask, (x, y), self.BrushSiz, cv2.GC_FGD, -1)
        elif event == cv2.EVENT_RBUTTONDOWN:
            cv2.circle(
                self.img_show, (x, y), self.BrushSiz, self.RColor, -1
            )  # 오른쪽 버튼을 클릭하면 빨간색
    ...
    def cutFunction(self):
        ...
        cv2.grabCut(
            self.img, self.mask, None, background, foreground, 5, cv2.GC_INIT_WITH_MASK
        )
