...
class VideoSpecialEffect(QMainWindow):
    def __init__(self):
        ...
        self.pickCombo.addItems(
            ["원래 영상", "흐릿(조금)", "흐릿(중간)", "흐릿(많이)", "빨강", "녹색", "파랑"]
        )
    ...
    def videoSpecialEffectFunction(self):
        ...
        while True:
            ...
            if pick_effect == 0:
                ...
                special_img = change_bg.blur_frame(
                    frame, moderate=True, detect="person"
                )
            elif pick_effect == 3:
                special_img = change_bg.blur_frame(frame, extreme=True, detect="person")
            elif pick_effect == 4:
                special_img = change_bg.color_frame(
                    frame, colors=(255, 0, 0), detect="person"
                )
            elif pick_effect == 5:
                special_img = change_bg.color_frame(
                    frame, colors=(0, 255, 0), detect="person"
                )
            elif pick_effect == 6:
                special_img = change_bg.color_frame(
                    frame, colors=(0, 0, 255), detect="person"
                )
